...
class TraceCorner(PntInc):
    def __init__(self, parent: AbstractTrace, segments: tuple[Segment, Segment], bend_radius: float):
        ...
        if bend_radius < parent.shape.radius:
            raise RuntimeError("Error in TraceCorner.min_corner_increment_angle: " + f"bend_radius {bend_radius} is less than the pipe shape radius {parent.shape.radius}!")
    ...
    def get_arc_properties(self) -> tuple[float, float, float, tuple[float, float]]:
        """ Get common properties about the arc that this trace corner follows.
        ...
        arc_length: float
            ...
            arc. The actual length will be different due to the trace corner being
    ...
    def get_center_line_points(self):
        """
        Get the points that define the path for the trace corner.

        These are the points along the "center line" of the trace corner,
        as pairs of (xy, θ), where:
            - xy is the x,y location of the point
            - θ is the direction that point is facing (0-2π)
        
        The first point in the trace corner corresponds to the end of the
        ...
        # Find the tangent from the arc center to the trace corner (aka the end of the first segment).
...
if __name__ == "__main__":
    ...
    unittests_out = open("tests/test_TraceCorners_regression_vals.json", "w")
